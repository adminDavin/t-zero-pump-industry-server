...
import com.t.zero.b.i.pump.service.PumpExportDxfConfigService;
...
@RestController
@RequestMapping("/pump_export_dxf_config")
public class PumpExportDxfConfigCnotroller extends TZeroBasicController {
	private final PumpExportDxfConfigService pumpExportDxfConfigService;

	public PumpExportDxfConfigCnotroller(ResponseExceptionHandler responseExceptionHandler, PumpExportDxfConfigService pumpExportDxfConfigService) {
		super(responseExceptionHandler);
		this.pumpExportDxfConfigService = pumpExportDxfConfigService;
	}
	...
	public ResponseResult<Object> list(@RequestHeader(value = Header.TENANT_ID) Integer tenantId,
		...
		try {
			return ResponseResult.ok(pumpExportDxfConfigService.list(CommonParams.build(tenantId, userId), content.getContent()));
		} catch (Exception e) {
	...
	public ResponseResult<Object> modify(@RequestHeader(value = Header.TENANT_ID) Integer tenantId,
			@RequestHeader(value = Header.USER_ID) Integer userId, @RequestBody ContentRequest content) {
		try {
			return ResponseResult.ok(pumpExportDxfConfigService.createOrModify(CommonParams.build(tenantId, userId), content.getContent()));
		} catch (Exception e) {
			return responseExceptionHandler.handle(String.format("ResInfoDefController", LIST), e);
		}
	}
	...
	public ResponseResult<Object> delete(@RequestHeader(value = Header.TENANT_ID) Integer tenantId,
			@RequestHeader(value = Header.USER_ID) Integer userId, @RequestBody ContentRequest content) {
		try {
			return ResponseResult.ok(pumpExportDxfConfigService.delete(CommonParams.build(tenantId, userId), content.getContent()));
		} catch (Exception e) {
			return responseExceptionHandler.handle(String.format("ResInfoDefController", LIST), e);
		}
	}
