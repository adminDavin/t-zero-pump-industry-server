...
import com.t.zero.b.i.pump.service.PumpInformationDataService;
...
@RestController
@RequestMapping("/pump_information_data")
public class PumpInformationDataController extends TZeroBasicController {
	private final PumpInformationDataService pumpInformationDataService;

	public PumpInformationDataController(ResponseExceptionHandler responseExceptionHandler, PumpInformationDataService pumpInformationDataService) {
		super(responseExceptionHandler);
		this.pumpInformationDataService = pumpInformationDataService;
	}
	...
	public ResponseResult<Object> list(@RequestHeader(value = Header.TENANT_ID) Integer tenantId,
		...
		try {
			return ResponseResult.ok(pumpInformationDataService.list(CommonParams.build(tenantId, userId), content.getContent()));
		} catch (Exception e) {
	...
	public ResponseResult<Object> get(@RequestHeader(value = Header.TENANT_ID) Integer tenantId,
			@RequestHeader(value = Header.USER_ID) Integer userId, @RequestBody ContentRequest content) {
		try {
			return ResponseResult.ok(pumpInformationDataService.get(CommonParams.build(tenantId, userId), content.getContent()));
		} catch (Exception e) {
			return responseExceptionHandler.handle(String.format("ResInfoDefController", LIST), e);
		}
	}
	...
	public ResponseResult<Object> modify(@RequestHeader(value = Header.TENANT_ID) Integer tenantId,
			@RequestHeader(value = Header.USER_ID) Integer userId, @RequestBody ContentRequest content) {
		try {
			return ResponseResult.ok(pumpInformationDataService.createOrModify(CommonParams.build(tenantId, userId), content.getContent()));
		} catch (Exception e) {
			return responseExceptionHandler.handle(String.format("ResInfoDefController", LIST), e);
		}
	}
	...
	public ResponseResult<Object> delete(@RequestHeader(value = Header.TENANT_ID) Integer tenantId,
			@RequestHeader(value = Header.USER_ID) Integer userId, @RequestBody ContentRequest content) {
		try {
			return ResponseResult.ok(pumpInformationDataService.delete(CommonParams.build(tenantId, userId), content.getContent()));
		} catch (Exception e) {
			return responseExceptionHandler.handle(String.format("ResInfoDefController", LIST), e);
		}
	}
